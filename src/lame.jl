...
function id3tag_init(lame::LAME)
    ccall((:id3tag_init, libmp3lame), Nothing, (LAME,), lame)
end

"""tell LAME to add ID3v2"""
function id3tag_add_v2(lame::LAME)
    ccall((:id3tag_add_v2, libmp3lame), Nothing, (LAME,), lame)
end

"""force using ID3v2 and not ID3v1"""
function id3tag_v2_only(lame::LAME)
    ccall((:id3tag_v2_only, libmp3lame), Nothing, (LAME,), lame)
end
...
function lame_set_write_id3tag_automatic(lame::LAME, v::Integer)
    ccall((:lame_set_write_id3tag_automatic, libmp3lame), Nothing,
          (LAME, Cint), lame, v)
end
